...
def run_kinematics(p, th_input):
    ...
    # Evaluamos solo los enlaces físicos para evitar fallos por offsets en 0
...
def fitness_function(p):
    ...
    # Penalización mínima para que priorice precisión pero no genere brazos de 5 metros
    penalty_size = 0.0001 * np.sum(p[:12]) 
    return (0.33 * err_ifp) + (0.33 * err_ifd) + (0.34 * err_tip) + penalty_size

# --- Espacio de Búsqueda (Restaurado a la coherencia física) ---
bounds = [
    (0.015, 0.10), (0.015, 0.10),                                # Bancada1, Bancada2 (Hasta 10cm)
    (0.01, 0.10), (0.01, 0.10), (0.01, 0.10), (0.01, 0.10), (0.01, 0.10), # Link 1-5
    (0.01, 0.10), (0.01, 0.10), (0.01, 0.10), (0.01, 0.10), (0.01, 0.10), # Link 6-10
    (0.0, 0.05), (0.0, 0.05),                                    # Offsets de la base
    (0, np.pi/2), (0, np.pi/4),                                  # Theta Aux (Restaurados al cuadrante positivo para evitar ensamble al revés)
    (1, 5),                                                      # Gear Ratio
...
def objective_optuna(trial):
    # Se regresó 'rand1bin' que es vital para la exploración profunda
    strategy = trial.suggest_categorical('strategy', ['best1bin', 'best2bin', 'rand1bin'])
    popsize = trial.suggest_int('popsize', 20, 50) 
    ...
    res = differential_evolution(
        ...
        tol=1e-3,
        polish=False,
        updating='deferred', # Restaurado a deferred para evitar la convergencia prematura
        workers=-1 
    )
    return res.fun
...
if __name__ == '__main__':
    print('\n====================================================')
    print('>> ETAPA 1: Búsqueda Profunda de Hiperparámetros')
    print('====================================================')
    ...
    print('\n====================================================')
    print('>> ETAPA 2: Optimización Cinemática Intensiva')
    print('====================================================')
    
    # Aquí el algoritmo se tomará su tiempo (maxiter=1000). Déjalo trabajar tranquilo.
    ...
    # --- Extracción y Reporte de Resultados ---
    ...
    # Calculo estricto del error geométrico final en MILÍMETROS
    ...
    print('====================================================')
    print(f">> ERROR GLOBAL DEL MECANISMO: {error_global_mm:.3f} mm")
    print(f"   - Error Articulación IFP:   {err_ifp_mm:.3f} mm")
    print(f"   - Error Articulación IFD:   {err_ifd_mm:.3f} mm")
    print(f"   - Error Punta del Dedo:     {err_tip_mm:.3f} mm\n")
    
    print('>> PARÁMETROS DIMENSIONALES OPTIMIZADOS (Metros / Radianes)')
    nombres_parametros = [
    ...
    print(f'   Traslación X (MOCAP): {t_opt[0]*1000:.2f} mm')
    print(f'   Traslación Y (MOCAP): {t_opt[1]*1000:.2f} mm')
    print(f'   Rotación Base (MOCAP): {angulo_montaje:.2f} grados')
    print('====================================================\n')

    # --- Gráficas (MOCAP vs Optimizado) ---
    plt.figure(figsize=(10, 8))
    
    # MOCAP Reales
    plt.plot(mocap_pts['ifp'][:,0]*1000, mocap_pts['ifp'][:,1]*1000, 'r--', lw=2.5, alpha=0.6, label='MOCAP IFP (Real)')
    plt.plot(mocap_pts['ifd'][:,0]*1000, mocap_pts['ifd'][:,1]*1000, 'g--', lw=2.5, alpha=0.6, label='MOCAP IFD (Real)')
    plt.plot(mocap_pts['tip'][:,0]*1000, mocap_pts['tip'][:,1]*1000, 'b--', lw=2.5, alpha=0.6, label='MOCAP Punta (Real)')
    
    # Simuladas y Optimizadas
    plt.plot(sim_aligned['ifp'][:,0]*1000, sim_aligned['ifp'][:,1]*1000, 'r-', lw=2, label='EXO IFP (Optimizado)')
    plt.plot(sim_aligned['ifd'][:,0]*1000, sim_aligned['ifd'][:,1]*1000, 'g-', lw=2, label='EXO IFD (Optimizado)')
    plt.plot(sim_aligned['tip'][:,0]*1000, sim_aligned['tip'][:,1]*1000, 'b-', lw=2, label='EXO Punta (Optimizado)')
