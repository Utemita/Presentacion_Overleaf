# VERSION 16 - CORRECCIÓN DE GANCHOS MEDIANTE PENALIZACIÓN SUAVE (SOFT CONSTRAINT)
import numpy as np
...
def run_kinematics(p, num_puntos):
    ...
    for th2 in th_input:
        ...
        # Filtro de singularidad extrema (Evita roturas del mecanismo)
        if len(PXtip) > 1:
...
# ==============================================================================
# --- 6. FUNCIÓN OBJETIVO CON PENALIZACIÓN SUAVE (SOFT CONSTRAINT) ---
# ==============================================================================
def fitness_function(p):
    ...
    # 1. ERROR DE FORMA GENERAL (Chamfer Distance)
    ...
    # 2. PENALIZACIÓN SUAVE DE EXTREMOS (Evita el "gancho" al final)
    # Solo "jalamos" la punta del dedo para guiar el trazo sin asfixiar a los demás eslabones
    error_inicio_punta = np.linalg.norm(sim_tip_aligned[0] - mocap_pts['tip'][0])
    error_fin_punta = np.linalg.norm(sim_tip_aligned[-1] - mocap_pts['tip'][-1])
    
    # Peso mínimo (0.1) para que solo actúe como desempate cuando dos curvas se parecen, 
    # prefiriendo la que no tiene "gancho".
    peso_suave = 0.1 

    # Cálculo final: Error Promedio + Anclaje Suave
    error_total = (0.20 * err_ifp) + (0.30 * err_ifd) + (0.50 * err_tip) + (peso_suave * (error_inicio_punta + error_fin_punta))
