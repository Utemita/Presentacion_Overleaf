...
# ==============================================================================
# --- 1. PARÁMETROS ANTROPOMÉTRICOS (longitudes de falanges reales) ---
# ==============================================================================
...
# ==============================================================================
# --- 2. CARGA Y CORRECCIÓN DE DATOS MOCAP ---
# ==============================================================================
# CORRECCIONES APLICADAS:
#   (a) Los datos del CSV vienen de flexión máxima → extensión; se invierten
...
#       del sensor o ligera hiperextensión fisiológica. Un ángulo DIP negativo
#       no puede replicarse con el exoesqueleto (límite físico del mecanismo),
#       por lo que se recorta a 0° como mínimo.
...
try:
    ...
    # (b) Recortar DIP a 0° mínimo (no puede existir hiperextensión en el exo)
    ...
    print("\n>> ERROR CRÍTICO: No se encontró 'mocap_indice_120pts.csv'.")
    raise

# Cinemática directa del dedo (cadena de cuerpos rígidos desde la MCF)
# Los ángulos son ACUMULADOS en el sistema global (igual que el código original,
# pero ahora los datos de entrada están limpios).
seg_prox = mcp_smooth                          # orientación falange proximal
seg_med  = mcp_smooth + pip_smooth             # orientación falange medial
seg_dist = mcp_smooth + pip_smooth + dip_smooth  # orientación falange distal
...
# Rango de entrada de la manivela principal (0 → 85°), igual que antes
theta_input = np.linspace(0, np.deg2rad(85), N_PUNTOS)

print(f">> MOCAP cargado: {N_PUNTOS} puntos.")
print(f"   MCP: {np.rad2deg(mcp_smooth[0]):.1f}° → {np.rad2deg(mcp_smooth[-1]):.1f}°")
print(f"   PIP: {np.rad2deg(pip_smooth[0]):.1f}° → {np.rad2deg(pip_smooth[-1]):.1f}°")
print(f"   DIP: {np.rad2deg(dip_smooth[0]):.1f}° → {np.rad2deg(dip_smooth[-1]):.1f}°")

# ==============================================================================
# --- 3. FUNCIONES DE EVALUACIÓN ---
# ==============================================================================
def chamfer_distance(curve_target, curve_sim):
    """Distancia de Chamfer bidireccional (métrica de forma)."""
...
def optimal_rigid_transform(target, sim):
    """Transformación rígida óptima (rotación + traslación) entre dos nubes de puntos."""
...
def monotonicity_penalty(curve):
    """
    Penaliza inversiones de dirección en la trayectoria simulada.
    Calcula la variación total y la compara con la longitud de arco neta;
    si hay reversiones, la diferencia es positiva.
    
    Devuelve un valor ≥ 0 (0 = curva perfectamente monótona en avance).
    """
    diffs = np.diff(curve, axis=0)
    arc_increments = np.linalg.norm(diffs, axis=1)
    total_arc = np.sum(arc_increments)
    
    # Proyección acumulada en la dirección dominante (componente principal)
    if total_arc < 1e-9:
        return 0.0
...
def run_kinematics(p, th_input):
    ...
    # CORRECCIÓN CLAVE: rs2 y theta_aux_s2 deben calcularse con FP_REAL
    # (longitud real de la falange proximal que el exoesqueleto debe seguir),
    # NO con el parámetro Bancada2 ni con ningún Link optimizable.
    ...
    for th2 in th_input:
        ...
        # Ángulo y posición de la articulación IFP
        ...
        # Ángulos de las falanges medial y distal
        theta_fm = theta4m2 + theta_aux_fm
        theta_fd = theta_fm + theta_aux_fd

        # -----------------------------------------------------------------
        # RESTRICCIÓN FÍSICA ANTI-GANCHO:
        # En un dedo real realizando un agarre, los ángulos de las falanges
        # medial y distal deben ser MONÓTONOS (nunca retroceden) a lo largo
...
# Peso de la penalización de monotonicidad (escala en metros)
W_MONO = 5.0
...
def fitness_function(p):
    ...
    # Penalización de monotonicidad sobre las trayectorias alineadas
...
# ==============================================================================
# --- 7. BOUNDS (sin cambios respecto al original) ---
# ==============================================================================
bounds = [
...
# ==============================================================================
# --- 8. OPTIMIZACIÓN BAYESIANA (Optuna) ---
# ==============================================================================
...
if __name__ == '__main__':
    ...
    print('\n====================================================')
    print('>> ETAPA 2: Optimización Cinemática Profunda (ED)')
    print('====================================================')
    ...
    if best_sim is None:
        print("\n>> ADVERTENCIA: El resultado final no produce una cinemática válida.")
        print("   Considere ampliar los bounds o aumentar maxiter.")
        exit()

    # --- Transformación de alineación final ---
    all_mocap = np.vstack([mocap_pts[k] for k in ('ifp', 'ifd', 'tip')])
    ...
    print('\n>> PARÁMETROS DE MONTAJE (transformación rígida)')
    print(f'   Traslación X : {t_opt[0]*1000:.2f} mm')
    print(f'   Traslación Y : {t_opt[1]*1000:.2f} mm')
    print(f'   Rotación Base: {angulo_montaje:.2f}°')
    print('====================================================\n')
    ...
    if len(mocap_pts['ifp']) == len(sim_aligned['ifp']):
        ...
        }).to_csv('Resultados_Alineados.csv', index=False)
        print(">> Archivos guardados: Parametros_Optimizados_Mecanismo.txt, "
              "Resultados_Alineados.csv, Resultados_Biofidelidad.png")
