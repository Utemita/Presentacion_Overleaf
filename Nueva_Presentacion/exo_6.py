...
# Ajustado a los valores del archivo CinematicaExoFinal.m
FP_REAL = 0.049  
FM_REAL = 0.026  
FD_REAL = 0.024  
...
def chamfer_distance(curve_target, curve_sim):
    """
    Calcula la distancia entre dos curvas sin importar la velocidad de la trayectoria,
...
def optimal_rigid_transform(target, sim):
    """
    Algoritmo de Kabsch (Procrustes rígido). Encuentra la rotación y traslación
    ideal para alinear la simulación con el MOCAP sin escalar el mecanismo.
    """
...
def run_kinematics(p, th_input):
    ...
    # Penalización rápida si las longitudes son negativas o muy pequeñas
    ...
    # Retornar como arrays apilados (N_PUNTOS, 2) para el análisis
    return {
...
# --- Función Objetivo con Alineación Procrustes ---
def fitness_function(p):
    sim_data = run_kinematics(p, theta_input)
    if sim_data is None: 
        return 1e6 # Penalización por bloqueo cinemático

    # Concatenamos todos los puntos para encontrar la mejor alineación global
    all_mocap = np.vstack((mocap_pts['ifp'], mocap_pts['ifd'], mocap_pts['tip']))
    all_sim = np.vstack((sim_data['ifp'], sim_data['ifd'], sim_data['tip']))

    # Obtener rotación y traslación ideal
    R, t = optimal_rigid_transform(all_mocap, all_sim)

    # Aplicar la transformación a las trayectorias
    ...
    # Evaluar forma de la trayectoria (Chamfer)
    ...
    # Penalización leve para evitar eslabones innecesariamente grandes
    penalty_size = 0.005 * np.sum(p[:12]) 

    # Damos más peso a la punta del dedo y articulación distal
...
if __name__ == '__main__':
    ...
    # Alinear trayectorias finales para graficar
    sim_aligned = {
