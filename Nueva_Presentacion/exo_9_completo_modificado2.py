# VERSION MODIFICADA PARA REDUCCION DE ERROR
import numpy as np
...
try:
    ...
    # Convertir los ángulos de grados a radianes para las funciones trigonométricas
    ...
    # Aseguramos que el número de puntos se adapte dinámicamente al archivo (120 puntos)
    N_PUNTOS = len(datos_mocap)
    
except FileNotFoundError:
    print("\n>> ERROR CRÍTICO: No se encontró el archivo 'mocap_indice_120pts.csv'.")
    print(">> Asegúrate de que el archivo generado en el paso anterior esté en esta carpeta.\n")
    exit()

# Definimos el vector de entrada del motor/manivela que accionará el mecanismo del exoesqueleto
# theta_input fijo reemplazado por theta_ini/theta_fin optimizados

# --- Cinemática Directa del Dedo Humano (Generación de la Ruta de Referencia) ---
# Ángulos acumulados absolutos respecto a la horizontal
angulo_pp = theta_mcp
angulo_mp = theta_mcp + theta_pip
angulo_dp = theta_mcp + theta_pip + theta_dip

# Cálculo de las posiciones espaciales (X, Y) del dedo en metros
...
# Guardamos las trayectorias espaciales reales en el diccionario objetivo 'mocap_pts'
mocap_pts = {
...
# ==============================================================================
def rmse_curve(real, sim):
    """Error RMS punto a punto."""
    return np.sqrt(np.mean(np.sum((real-sim)**2, axis=1)))
...
# ==============================================================================
# --- 4. SOLUCIONES MATEMÁTICAS DE MECANISMOS ---
# ==============================================================================
...
# ==============================================================================
# --- 5. MODELO CINEMÁTICO DEL EXOESQUELETO ---
# ==============================================================================
...
def objective_optuna(trial):
    ...
    res = differential_evolution(
        ...
        maxiter=30,  # Aumenta a 50 o 100 si tienes una buena computadora
        tol=1e-3,
        polish=False,
        updating='deferred',
        workers=-1  # Cambia a 1 si tienes problemas de multiprocesamiento en Windows
    )
...
if __name__ == '__main__':
    ...
    resultado = differential_evolution(
        ...
        disp=True,
        updating='deferred',
        workers=-1  # Cambia a 1 si tienes problemas de multiprocesamiento
    )
