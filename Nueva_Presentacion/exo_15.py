# VERSION DEFINITIVA - CORRECCIÓN DE "GANCHOS" Y DEFLEXIONES (ANCLAJE DE FRONTERAS)
import numpy as np
...
def run_kinematics(p, num_puntos):
    ...
    for th2 in th_input:
        ...
        # Filtro físico básico contra singularidades que rompen el dedo
        if len(PXtip) > 1:
...
# ==============================================================================
# --- 6. FUNCIÓN OBJETIVO MODIFICADA (CON PENALIZACIÓN DE DEFLEXIONES) ---
# ==============================================================================
def fitness_function(p):
    ...
    # 1. ERROR DE FORMA GENERAL (Chamfer)
    ...
    # 2. PENALIZACIÓN DE FRONTERAS (Evita que el algoritmo sacrifique los extremos)
    error_inicio = (
        np.linalg.norm(sim_ifp_aligned[0] - mocap_pts['ifp'][0]) + 
        np.linalg.norm(sim_ifd_aligned[0] - mocap_pts['ifd'][0]) + 
        np.linalg.norm(sim_tip_aligned[0] - mocap_pts['tip'][0])
    )

    error_fin = (
        np.linalg.norm(sim_ifp_aligned[-1] - mocap_pts['ifp'][-1]) + 
        np.linalg.norm(sim_ifd_aligned[-1] - mocap_pts['ifd'][-1]) + 
        np.linalg.norm(sim_tip_aligned[-1] - mocap_pts['tip'][-1])
    )
    peso_extremos = 3.0 # Fuerte peso para forzar anclaje

    # 3. FILTRO ANTI-GANCHOS (Singularidades cinemáticas suaves)
    saltos_ifp = np.linalg.norm(np.diff(sim_ifp_aligned, axis=0), axis=1)
    saltos_ifd = np.linalg.norm(np.diff(sim_ifd_aligned, axis=0), axis=1)
    
    # Si detecta un salto mayor a 5 mm entre dos puntos consecutivos de la simulación, castiga el mecanismo
    if len(saltos_ifp) > 0 and (np.max(saltos_ifp) > 0.005 or np.max(saltos_ifd) > 0.005):
        return 1e6

    # Cálculo final: Error Promedio + Anclaje Fuerte en los Extremos
    error_total = (0.20 * err_ifp) + (0.30 * err_ifd) + (0.50 * err_tip) + (peso_extremos * (error_inicio + error_fin))
    return error_total
