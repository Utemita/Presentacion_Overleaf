...
%---------------- Indice automatico ----------------%
\begin{frame}{Indice (1/3)}
\scriptsize
\tableofcontents[sections={1-4}]
\end{frame}

\begin{frame}{Indice (2/3)}
\scriptsize
\tableofcontents[sections={5-6}]
\end{frame}

\begin{frame}{Indice (3/3)}
\scriptsize
...
\begin{frame}{Resultados: Hiperheur\'istica + AG}
    ...
    \quad \textbf{Error RMS final: 0.006 m} (no satisfactorio).

    \begin{figure}
        \centering
        \includegraphics[width=0.8\linewidth]{imagens/resultado_hiper_AG.png}
    ...
    \begin{enumerate}
        \item Ecuaciones de cierre sin soluci\'on frecuente (penalizaci\'on $10^6$); alta sensibilidad param\'etrica.
        \item Costo computacional excesivo y eslabones con longitudes no realistas ($>$10\,cm).
        \item Hiperheur\'istica insuficiente para equilibrar trayectoria natural y restricciones mec\'anicas.
    \end{enumerate}
...
%-------------------------------------------------------%
\subsection{Evoluci\'on Diferencial + Optuna}

% NUEVAS DIAPOSITIVAS: DE + Optuna
%-------------------------------------------------------%
\begin{frame}{Evoluci\'on Diferencial vs.\ Algoritmo Gen\'etico}
    \scriptsize
    \textbf{?`Por qu\'e DE en lugar de AG?} \cite{price2005differential, deb2012optimization}
    \vspace{0.2cm}

    \begin{columns}[T]
    \begin{column}{0.48\textwidth}
        \textbf{Algoritmo Gen\'etico (AG):}
        \begin{itemize}
            \item Codificaci\'on binaria o real con operadores gen\'ericos
            \item Mutaci\'on y cruce no adaptativos
            \item Convergencia lenta en espacios continuos de alta dimensi\'on
        \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
        \textbf{Evoluci\'on Diferencial (DE):}
        \begin{itemize}
            \item Opera directamente sobre vectores continuos (18 variables)
            \item Mutaci\'on auto-adaptativa mediante vectores diferencia
            \item Mejor exploraci\'on en espacios de alta dimensi\'on
            \item Convergencia probada para optimizaci\'on de mecanismos
        \end{itemize}
    \end{column}
    \end{columns}
    ...
    \vspace{0.3cm}
    \begin{center}
    \textbf{Resultado:} DE logr\'o error sub-milim\'etrico donde AG se estanc\'o en 6\,mm.
    \end{center}
\end{frame}

%-------------------------------------------------------%
\begin{frame}{Optuna: Optimizaci\'on Bayesiana de Hiperpar\'ametros}
    \scriptsize
    \textbf{Estrategia de dos etapas} \cite{akiba2019optuna}:
    \vspace{0.2cm}

    \textbf{Etapa 1 --- Optuna (TPE Bayesiano):}
    \begin{itemize}
        \item Busca la mejor combinaci\'on de hiperpar\'ametros para DE:
        \begin{itemize}
            \scriptsize
            \item Estrategia: \texttt{best1exp}, \texttt{rand1bin}, \texttt{best1bin}
            \item Tama\~no de poblaci\'on: 15--30
            \item Rango de mutaci\'on: $[0.5, 1.5]$
            \item Recombinaci\'on: $[0.5, 0.9]$
        \end{itemize}
    \end{itemize}

    \vspace{0.2cm}
    \textbf{Etapa 2 --- DE con hiperpar\'ametros \'optimos:}
    \begin{itemize}
        \item Ejecuci\'on completa (1000 iteraciones) con los mejores par\'ametros
        \item Polish con L-BFGS-B para refinamiento local
    \end{itemize}

    \vspace{0.2cm}
    \begin{center}
    \fbox{\parbox{0.85\textwidth}{\centering
    Optuna elimina el ajuste manual de hiperpar\'ametros, reduciendo\\
    significativamente el tiempo de desarrollo y mejorando la repetibilidad.
    }}
    \end{center}
\end{frame}

%-------------------------------------------------------%
\begin{frame}{Distancia de Chamfer como M\'etrica de Forma}
    \scriptsize
    \textbf{?`Por qu\'e Chamfer en lugar de RMS punto a punto?} \cite{borgefors1988hierarchical, fan2017chamfer}
    \vspace{0.2cm}
    ...
    \vspace{0.1cm}
    \textbf{Ventajas sobre RMS:}
    \begin{itemize}
        \item \textbf{Bidireccional:} mide similitud de forma en ambas direcciones
        \item \textbf{Robusta:} maneja diferentes densidades de puntos entre MOCAP y simulaci\'on
        \item \textbf{Tolerante:} menos sensible a desalineamientos locales temporales
    \end{itemize}

    \vspace{0.2cm}
    \textbf{Complementos en la funci\'on objetivo:}
    \begin{itemize}
        \item Alineaci\'on de Procrustes (rotaci\'on + traslaci\'on \'optima)
        \item Penalizaci\'on de monotonicidad (evita ``ganchos'' en trayectorias)
        \item Pesos: $W_{IFP}=0.25$, $W_{IFD}=0.375$, $W_{Tip}=0.375$
    \end{itemize}
\end{frame}

%-------------------------------------------------------%
\begin{frame}[fragile]{C\'odigo: Funci\'on Objetivo (\texttt{exo\_17.py})}
...
%-------------------------------------------------------%
\begin{frame}{Resultados: DE + Optuna}
    \scriptsize
    \begin{figure}
        \centering
        \includegraphics[width=0.70\linewidth]{imagens/resultado_de_optuna.png}
        \caption{Trayectorias MOCAP vs.\ EXO optimizado con DE + Optuna}
        \label{fig:resultado_de_optuna}
    \end{figure}
    ...
    \begin{column}{0.45\textwidth}
        \tiny
        \textbf{AG + Hiperheur\'istica:}
        \begin{itemize}
            \item Error: 6\,mm (RMS)
            \item Eslabones no realistas
            \item Convergencia inestable
        \end{itemize}
    \end{column}
    \begin{column}{0.45\textwidth}
        \tiny
        \textbf{DE + Optuna + Chamfer:}
        \begin{itemize}
            \item Error global: $<$1\,mm
            \item Eslabones dentro de bounds
            \item Trayectorias sin ``ganchos''
        \end{itemize}
    \end{column}
    \end{columns}

    \begin{center}
    $\Rightarrow$ \textbf{Mejora de $>$6$\times$ en precisi\'on con restricciones f\'isicas satisfechas.}
    \end{center}
\end{frame}
...
\begin{frame}{Conclusiones}
    \begin{itemize}
        \item Se implement\'o un Algoritmo Gen\'etico con hiperheur\'istica para la optimizaci\'on de mecanismos de 4 y 5 barras, y posteriormente se mejor\'o con Evoluci\'on Diferencial + Optuna.
        \item El m\'etodo de Procrustes mejor\'o significativamente la alineaci\'on entre trayectorias, reduciendo el error RMS.
        \item La combinaci\'on DE + Optuna + Distancia de Chamfer logr\'o un error global menor a 1\,mm, superando en m\'as de 6$\times$ al enfoque previo con AG.
        ...
        \item En conjunto, los resultados demuestran que la optimizaci\'on metaheur\'istica combinada con m\'etricas robustas de forma es viable para el dise\~no dimensional de exoesqueletos de rehabilitaci\'on.
    \end{itemize}
\end{frame}
...
\begin{frame}{Siguientes Pasos}
    \begin{itemize}
        \item Se est\'a armando el modelo CAD con las nuevas medidas para despu\'es validar los resultados del estudio de movimiento con lo obtenido con el algoritmo de optimizaci\'on y volver a armarlo en f\'isico con estas nuevas medidas.
